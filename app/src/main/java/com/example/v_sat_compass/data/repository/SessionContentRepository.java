...
public class SessionContentRepository {

    private final ExamApi api;
    private final Gson gson = new Gson();

    public SessionContentRepository() {
        this(ApiClient.getClient().create(ExamApi.class));
    }

    public SessionContentRepository(ExamApi api) {
        this.api = api;
    }

    public interface RepositoryCallback<T> {
        void onSuccess(T data);
        void onError(SessionContentError error);
    }

    public static class SessionContentError {
        public enum Type {
            HTTP,
            SERVER,
            NETWORK
        ...
        public SessionContentError(Type type, int statusCode, String code, String message) {
            this.type = type;
    ...
    public void getQuestion(
            long sessionId,
            long questionId,
            RepositoryCallback<SessionQuestionContentResponse> callback
    ) {
        enqueue(api.getSessionQuestionContent(sessionId, questionId), callback);
    }

    public void getAnswerKeys(
            long sessionId,
            RepositoryCallback<SessionAnswerKeysResponse> callback
    ) {
        enqueue(api.getSessionAnswerKeys(sessionId), callback);
    }
    ...
    private <T> void enqueue(Call<ApiResponse<T>> call, RepositoryCallback<T> callback) {
        call.enqueue(new Callback<ApiResponse<T>>() {
            ...
            public void onResponse(Call<ApiResponse<T>> call, Response<ApiResponse<T>> response) {
                if (response.isSuccessful()) {
                    ...
                    if (body != null && body.isSuccess() && body.getData() != null) {
                        ...
                        callback.onError(new SessionContentError(
                                SessionContentError.Type.SERVER,
                                response.code(),
                                errorCode(body),
            ...
            public void onFailure(Call<ApiResponse<T>> call, Throwable t) {
                callback.onError(new SessionContentError(
                        SessionContentError.Type.NETWORK,
                        0,
                        "NETWORK_FAILURE",
                        t.getMessage()
                ));
    ...
    private SessionContentError toHttpError(Response<?> response) {
        ApiResponse<?> errorBody = parseErrorBody(response);
        SessionContentError.Type type = response.code() >= 500
                ? SessionContentError.Type.SERVER
                : SessionContentError.Type.HTTP;
        return new SessionContentError(
                type,
                response.code(),
                errorCode(errorBody),
